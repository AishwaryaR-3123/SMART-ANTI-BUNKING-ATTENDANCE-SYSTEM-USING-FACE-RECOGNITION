...
def student_dashboard():
    ...
    if student_record:
        # Extract dynamic date columns (keeping your original exact logic)
        #date_cols = [col for col in student_record.keys() if col not in ('regno', 'name')]
        
        # Securely fetch dictionary keys regardless of MySQL casing
        regno_key = 'RegNo' if 'RegNo' in student_record else 'regno'
        name_key = 'Name' if 'Name' in student_record else 'name'
        
        regno = student_record[regno_key]
        name = student_record[name_key]
        
        # --- THE FIX FOR ISSUE 2 ---
        # Strictly filter: Only count columns that start with a digit (like 09-03-26)
        # This prevents it from accidentally counting 'id', 'dept', etc.
        date_cols = [
            col for col in student_record.keys() 
            if col.lower() not in ('regno', 'name') and str(col)[0].isdigit()
        ]
        ...
        # --- NEW UI: Image on the left, Stats on the right ---
        col_img, col_info = st.columns([1, 3])
        
        with col_img:
            # Construct the image path using the exact RegNo from the database
            student_regno = student_record['RegNo']
            image_path = f"Face_Encodings/{student_regno}.jpg"
            
            if os.path.exists(image_path):
                st.image(image_path, width=150, caption="Profile Photo")
            else:
                st.warning("Photo not found.")
                
        with col_info:
            # Displaying your original headers and metrics
            st.subheader(f"RegNo: {student_record['RegNo']} | Name: {student_record['Name']}")
            
            c1, c2, c3, c4 = st.columns([1.5,1,1,1.2])
            c1.metric("Total Working Days", total_days)
            c2.metric("Days Present", days_present)
            c3.metric("Days Absent", days_absent)
            c4.metric("Attendance %", f"{attendance_percentage:.1f}%")
